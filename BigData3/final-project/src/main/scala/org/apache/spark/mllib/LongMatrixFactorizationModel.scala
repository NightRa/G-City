...
/**
  ...
  */
@Since("0.8.0")
class LongMatrixFactorizationModel @Since("0.8.0")(
                                                    @Since("0.8.0") val rank: Int,
                                                    @Since("0.8.0") val userFeatures: RDD[(Long, Array[Double])],
                                                    @Since("0.8.0") val productFeatures: RDD[(Long, Array[Double])])
  ...
  /**
    ...
    */
  private[this] def countApproxDistinctUserProduct(usersProducts: RDD[(Long, Long)]): (Long, Long) = {
    ...
    val aggregated = usersProducts.aggregate((zeroCounterUser, zeroCounterProduct))(
      (hllTuple: (HyperLogLogPlus, HyperLogLogPlus), v: (Long, Long)) => {
    ...
    if (usersCount < productsCount) {
      ...
      users.join(productFeatures).map {
        case (product, ((user, uFeatures), pFeatures)) =>
          Rating(user, product, blas.ddot(uFeatures.length, uFeatures, 1, pFeatures, 1).toFloat)
      }
    } else {
  ...
  /**
    * Java-friendly version of [[LongMatrixFactorizationModel.predict]].
    */
  @Since("1.2.0")
  def predict(usersProducts: JavaPairRDD[JavaLong, JavaLong]): JavaRDD[Rating[Long]] = {
    predict(usersProducts.rdd.asInstanceOf[RDD[(Long, Long)]]).toJavaRDD()
  }
  ...
  /**
    ...
    */
  @Since("1.1.0")
  def recommendProducts(user: Long, num: Int): Array[Rating[Long]] =
    LongMatrixFactorizationModel.recommend(userFeatures.lookup(user).head, productFeatures, num)
      .map(t => Rating(user, t._1, t._2.toFloat))

  def recommendAllProducts(user: Long): RDD[Ranking] = {
    LongMatrixFactorizationModel.allRecommendations(userFeatures.lookup(user).head, productFeatures).zipWithIndex().map {
  ...
  /**
    ...
    */
  @Since("1.1.0")
  def recommendUsers(product: Long, num: Int): Array[Rating[Long]] =
    LongMatrixFactorizationModel.recommend(productFeatures.lookup(product).head, userFeatures, num)
      .map(t => Rating(t._1, product, t._2.toFloat))
  ...
  override def save(sc: SparkContext, path: String): Unit = {
    LongMatrixFactorizationModel.SaveLoadV1_0.save(this, path)
  }
  ...
  /**
    ...
    */
  @Since("1.4.0")
  def recommendProductsForUsers(num: Int): RDD[(Long, Array[Rating[Long]])] = {
    LongMatrixFactorizationModel.recommendForAll(rank, userFeatures, productFeatures, num).map {
      case (user, top) =>
        val ratings = top.map { case (product, rating) => Rating(user, product, rating.toFloat) }
        (user, ratings)
    }
  }
  ...
  /**
    ...
    */
  @Since("1.4.0")
  def recommendUsersForProducts(num: Int): RDD[(Long, Array[Rating[Long]])] = {
    LongMatrixFactorizationModel.recommendForAll(rank, productFeatures, userFeatures, num).map {
      case (product, top) =>
        val ratings = top.map { case (user, rating) => Rating(user, product, rating.toFloat) }
        (product, ratings)
    }
  }
}
...
@Since("1.3.0")
object LongMatrixFactorizationModel extends Loader[LongMatrixFactorizationModel] {

  import org.apache.spark.mllib.util.Loader._

  def allRecommendations(recommendToFeatures: Array[Double],
                         recommendableFeatures: RDD[(Long, Array[Double])]): RDD[(Long, Double)] = {
    allRecommendationsUnordered(recommendToFeatures, recommendableFeatures).sortBy(-_._2)
  }

  private def allRecommendationsUnordered(recommendToFeatures: Array[Double],
                                          recommendableFeatures: RDD[(Long, Array[Double])]) = {
    recommendableFeatures.map { case (id, features) =>
  ...
  private def recommendForAll(
                               ...
                               srcFeatures: RDD[(Long, Array[Double])],
                               dstFeatures: RDD[(Long, Array[Double])],
                               num: Int): RDD[(Long, Array[(Long, Double)])] = {
    val srcBlocks = blockify(rank, srcFeatures)
    val dstBlocks = blockify(rank, dstFeatures)
    val ratings = srcBlocks.cartesian(dstBlocks).flatMap {
      case ((srcIds, srcFactors), (dstIds, dstFactors)) =>
  ...
  private def blockify(
                        rank: Int,
                        features: RDD[(Long, Array[Double])]): RDD[(Array[Long], DenseMatrix)] = {
  ...
  /**
    ...
    */
  @Since("1.3.0")
  override def load(sc: SparkContext, path: String): LongMatrixFactorizationModel = {
  ...
  object SaveLoadV1_0 {
    ...
    private[mllib]
    val thisClassName = "org.apache.spark.mllib.MatrixFactorizationModel"

    /**
      * Saves a [[LongMatrixFactorizationModel]], where user features are saved under `data/users` and
      * product features are saved under `data/products`.
      */
    def save(model: LongMatrixFactorizationModel, path: String): Unit = {
    ...
    def load(sc: SparkContext, path: String): LongMatrixFactorizationModel = {
      ...
      val userFeatures = sqlContext.read.parquet(userPath(path))
        .map { case Row(id: Long, features: Seq[_]) =>
      ...
      new LongMatrixFactorizationModel(rank, userFeatures, productFeatures)
    }
...
case class Ranking(user: Long, movie: Long, rating: Float, ranking: Int)
