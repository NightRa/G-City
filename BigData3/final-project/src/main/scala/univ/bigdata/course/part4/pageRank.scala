...
object PageRank {
  ...
  def execute(inputFile: String): Unit = {
    ...
    val ordering: Ordering[(Double, String)] =
      Ordering.by(vertex => (-vertex._1, vertex._2))

    // get top 100 users after sorting
    val topUsers = ranks.join(graph.vertices).map(_._2).takeOrdered(topK)(ordering)
